...
GENESIS_FILE="$HOME"/.bluechip/config/genesis.json
if [ -f "$GENESIS_FILE" ]; then
  echo "$GENESIS_FILE exists..."
else
  echo "$GENESIS_FILE does not exist. Generating..."

  bluechipd init --chain-id "$CHAIN_ID" "$MONIKER"
  ...
  sed -i 's/keyring-backend = "os"/keyring-backend = "test"/' "$HOME"/.bluechip/config/client.toml
fi

APP_TOML_CONFIG="$HOME"/.bluechip/config/app.toml
APP_TOML_CONFIG_NEW="$HOME"/.bluechip/config/app_new.toml
CONFIG_TOML_CONFIG="$HOME"/.bluechip/config/config.toml
...
if ! bluechipd keys show validator $KEYRING; then
  (echo "$PASSWORD"; echo "$PASSWORD") | bluechipd keys add validator $KEYRING

  # hardcode the validator account for this instance
  echo "$PASSWORD" | bluechipd add-genesis-account validator "1000000000$STAKE,1000000000$FEE" $KEYRING

  # (optionally) add a few more genesis accounts
  for addr in "$@"; do
    echo $addr
    bluechipd add-genesis-account "$addr" "1000000000$STAKE,1000000000$FEE"
  done

  # submit a genesis validator tx
  ## Workraround for https://github.com/cosmos/cosmos-sdk/issues/8251
  (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | bluechipd gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID" --amount="250000000$STAKE" $KEYRING
  ## should be:
  # (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | bluechipd gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID"
