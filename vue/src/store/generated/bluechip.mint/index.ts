import { Client, registry, MissingWalletError } from 'Smartdev0328-bluechip-client-ts'

import { Minter } from "Smartdev0328-bluechip-client-ts/bluechip.mint/types"
import { Params } from "Smartdev0328-bluechip-client-ts/bluechip.mint/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: bluechip.mint initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.bluechipMint.query.queryParams()).data
		...
		async QueryInflation({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.bluechipMint.query.queryInflation()).data
		...
		async QueryAnnualProvisions({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.bluechipMint.query.queryAnnualProvisions()).data
